...
// ============================================
// Mock AI layer â€” swap for OpenAI in production
// ============================================

const TELPLATESª Record<string, string[]> = {
  professional: [
...
Just bumping this to the top of your inbox. I'd love to chat about how {business} can help {company} grow.\n\nGot a few minutes this week?\n\n{signature}",
    "Hi {name}!\n\nHoping to connect with you à€” I think there's a real opportunity for us to work together.\n\nLet me know if you'd like to hop on a quick call!\n\nCheers,\n{signature}",
  ],
  casual: [
    "Hey {name},\n\nQuick follow-up! Would love to chat when you get a chance.\n\n"{signature}",
    "Hi {name} â€” just checkin' in! Let me know if you'd like to connect.\n\n${signature}",
  ],
  urgent: [
    ...
    "{name},\n\nFinal follow-up â€” I have a slot open this week that I'm holding for {company}. Let me know if you'd like to grab it.\n\n${signature}",
  ],
};
...
export async function generateFollowUp(
  ...
  const subjects = [
    `Following up â€” ${settings.business_name || "Quick question"}`,
    `Re: ${lead.company || lead.name} â€” next steps?`,
    `Checking in, ${lead.name.split(" ")[0]}`,
    `Don't want you to miss this, ${lead.name.split(" ")[0]}`,
    `Last follow-up â€” ${settings.business_name || ""}`,
  ];
...
export async function generateInitialOutreach(
  ...
  {signature} å½Ñ¡•È½ÁÑ¥½¹Ì(€½¹ÍĞÍÕ‰©•Ğ€ô€‘í¹…µ•ô°ÅÕ¥¬ÅÕ•ÍÑ¥½¸…‰½ÕĞ€‘í±•…¹½µÁ…¹äñğ€‰å½ÕÈ‰ÕÍ¥¹•ÍÌ‰õ€ì(€½¹ÍĞ‰½‘ä€ô!¤€‘í¹…µ•ô±q¹q¹$…µ”…É½ÍÌ€‘í±•…¹½µÁ…¹äñğ€‰å½ÕÈÁÉ½™¥±”‰ô…¹Ñ¡½Õ¡Ğ€‘í‰ÕÍ¥¹•ÍÍôµ¥¡Ğ‰”…‰±”Ñ¼¡•±À¸]”€‘í‘•Íô¹q¹q¹]½Õ±å½Ô‰”½Á•¸Ñ¼„ÅÕ¥¬€ÄÔµµ¥¹ÕÑ”…±°Ñ¡¥Ìİ••¬ıq¹q¹	•ÍĞ±q¸‘íÍ•ÑÑ¥¹Ì¹Í¥¹…ÑÕÉ”ñğ‰ÕÍ¥¹•ÍÍõ€ì((€É•ÑÕÉ¸ìÍÕ‰©•Ğ°‰½‘äôì)ô